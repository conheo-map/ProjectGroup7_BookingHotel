...
async function previewAndSubmit(input) {
    ...
    const container = document.querySelector('.suaphong-tai-khu-noidung');
    let originalContent = '';
    if (container) {
        originalContent = container.innerHTML;
        container.innerHTML = 
            '<i class="fas fa-spinner fa-spin fa-2x text-primary mb-2"></i>' +
            '<p class="mb-0 fw-bold">Đang tải lên...</p>' +
    ...
    if (!form) {
        alert("Lỗi hệ thống: Không tìm thấy form upload. Vui lòng tải lại trang.");
    ...
    try {
        ...
        if (result.success) {
            if (container) {
                container.innerHTML = '<i class="fas fa-check-circle fa-2x text-success mb-2"></i><p class="fw-bold">Xong!</p>';
            }
